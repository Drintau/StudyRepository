...
##### UML类图
![简单工厂模式](images/ms/MS321.png)
##### 优缺点和结论
1. 优点：简单易懂。
2. 缺点：工厂类单一，代码臃肿，违背高聚合原则。
3. 缺点：新增一个产品，就要增加一个具体产品类，修改工厂逻辑；增加系统的复杂度，违背“开闭原则”。
4. 结论：对于产品种类相对较少的情况，考虑使用简单工厂模式。
#### 3.2.2 工厂方法模式
##### UML类图
![工厂方法模式](images/ms/MS322.png)
##### 优缺点和结论
1. 优点：活性增强，创建新产品只需多写一个相应的产品类、工厂类，无需修改之前的代码。
2. 优点：典型的解耦框架；满足迪米特法则、依赖倒置原则和里氏替换原则。
3. 缺点：类的个数容易过多；增加了系统的抽象性和理解难度；抽象产品只能生产一种产品（抽象工厂模式解决）。
4. 结论：扩展性比简单工厂模式好。